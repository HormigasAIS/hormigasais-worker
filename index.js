  ...
  async fetch(request, env) {
    ...
    if (path.length === 0) {
      return json({
        ...
        protocolo: "LBH v1.1",
        endpoints: ["/push", "/get", "/all", "/consensus"]
      })
    }
    ...
    if (path[0] === "push" && request.method === "POST") {
      ...
      await env.PHEROMONES.put(node, JSON.stringify(payload), {
        expirationTtl: 600
      })
    ...
    if (path[0] === "get") {
      const node = path[1]
      const data = await env.PHEROMONES.get(node)
      if (!data) return json({ node, status: "no_data" })
    ...
    return json({ error: "Ruta no válida" })
  }
}
